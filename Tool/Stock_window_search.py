...
class Winsearch:
    ...
    def win_number(self, scale):
        ...
        if subdir+'_parameters' not in os.listdir(self.root + 'results/'):
            os.makedirs(self.root + 'results/' + subdir + '_parameters/')
        if 'images' not in os.listdir(self.root):
            os.mkdir(self.root + 'images/')

        path_to_json = self.root + 'results/' + subdir + '_parameters/'
        ...
        if 'data_variation_scale_' + scale + '.json' not in os.listdir(path_to_json):
            ...
            for window_size in tqdm(range(self.win_start, self.win_end)):
                ...
                [Z, variation, regime_number] = gen.stock_get_timeseries_variation(split_series, indices, window_size)
        ...
        tresh = mld_cut
        print('window size is ' + str(tresh))
        #mapping_grid_plot(dynamic=dynamic, window_sizes=window_sizes).series_variation(name=name, root=self.root)
